...
Before starting, verify that the Orchestrator has provided an artifact repository root initialised by `tools/artifacts.py` and a confirmed repository context. All Step 1 outputs are written relative to the artifact root. %% safety measure
...
Each selected case must be cited by its exact URL from the oracle results file. %% retraceablility

## 5 — Assemble the Feature Brief for Step 3
...
The selected community cases travel into Step 3 so the Implementer can ground the feature contract in real usage patterns.
